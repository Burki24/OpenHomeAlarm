...
Im Konfigurationsformular können die globale **Ausgangsverzögerung** und **Eingangsverzögerung** in Sekunden festgelegt werden. Die optionale Aktion **Bei Countdown-Schritt** ist eine normale Symcon-Aktion und läuft bei jeder Sekunde einer aktiven Ein- oder Ausgangsverzögerung. Damit können beispielsweise eine Restzeit angesagt, ein Gong ausgelöst oder ein Statuswert gesetzt werden. Bleibt das Aktionsfeld leer, geschieht nichts. Im Abschnitt **Code-Schutz** kann optional ein vier- bis achtstelliger **Unscharfschaltcode** hinterlegt werden. **Automatische Scharfschaltung** verwaltet wöchentliche Schaltpläne mit Wochentagen, Uhrzeit und Zielmodus. Im Abschnitt **Alarmaktionen** werden die **Alarmdauer** und alle Alarmreaktionen ausschließlich als **Alarm-Eskalationsaktionen** konfiguriert. Im Abschnitt **Systemüberwachung** wird außerdem das Intervall der **Sensor-Integritätsprüfung** festgelegt; dort stehen auch die optionalen Aktionen **Bei neuer Störung** und **Bei behobener Störung** bereit. Beide sind normale Symcon-Aktionen, werden jeweils nur beim Zustandswechsel einmal ausgeführt und können leer bleiben. Darunter steht die Liste **Sensoren und Auslöser** zur Verfügung. Dort kann ein Sensor zusätzlich als **Ausgangsweg** markiert werden. Ein Eintrag verweist direkt auf eine vorhandene Symcon-Variable und ist damit unabhängig vom Hersteller oder Protokoll des eigentlichen Geräts.
