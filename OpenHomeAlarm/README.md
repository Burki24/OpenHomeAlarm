...
Das Dashboard zeigt die Bereitschaft und aktuellen Blockierer für **Zuhause**, **Abwesend** und **Nacht** dauerhaft als kompakte Statuskarten. Im unscharfen Zustand können bereite Modi direkt scharfgeschaltet werden; im scharfen Zustand bleiben dieselben Karten als Überwachungsübersicht sichtbar und der aktive Modus wird hervorgehoben. Zusätzlich fasst eine kompakte Statuszeile Scharfmodus, Überwachung, Systemstatus und Alarmgedächtnis zusammen. Auf ausreichend breiten Kacheln ist das Codepad dauerhaft als fester Bestandteil der Alarmzentrale sichtbar. Solange keine Code-Eingabe benötigt wird, bleibt es vollständig sichtbar, aber deaktiviert. Sobald die Anlage mit aktiviertem Code-Schutz unscharf geschaltet werden kann, wird dasselbe Codepad aktiv und bietet unter den Zifferntasten einen eindeutigen **Anlage unscharf schalten**-Button. Unterhalb einer Kachelbreite von 900 Pixeln wird das feste Codepad ausgeblendet; dort öffnet **Mit Code unscharf schalten** weiterhin das kompakte Popup-Codepad. Ist kein Unscharfschaltcode konfiguriert, steht unabhängig von der Breite die direkte Unscharfschaltung zur Verfügung. Die Code-Eingabe bleibt ausschließlich temporär im JavaScript-Speicher der geöffneten Darstellung, wird nicht in einer Symcon-Variable abgelegt und nach Absenden, Abbrechen oder erfolgreichem Unscharfschalten sofort verworfen. Falsche Codes werden direkt am aktiven Codepad gemeldet, ohne den eingegebenen Code anzuzeigen oder zu protokollieren.
