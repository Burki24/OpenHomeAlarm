...
OpenHomeAlarm verwendet für externe Reaktionen die nativen Symcon-Aktionen. Optionale Aktionen sind standardmäßig deaktiviert, damit eine vollständig gültige Konfiguration auch ohne Aktionsauswahl möglich bleibt. Nach Auswahl **Aktion konfigurieren** und einmaligem Übernehmen wird der jeweilige native Aktionsdialog eingeblendet. Solange die Option auf **Keine Aktion** steht, ist das `SelectAction`-Element nicht Bestandteil des Konfigurationsformulars und kann daher andere Änderungen nicht blockieren. Im Feld **Bei Alarm** kann anschließend ein beliebiges Ziel samt passender Aktion gewählt werden. Die Aktion wird genau einmal beim Übergang von keinem aktiven Bereichsausgang zum ersten aktiven Bereichsausgang ausgeführt. Weitere gleichzeitig alarmierende Bereiche werden intern ergänzt, ohne dieselbe gemeinsame Einschaltaktion erneut auszuführen. Gleichzeitig wird `AlarmOutputActive` als Gesamtausgabe aktiv.

Die **Alarmdauer** legt fest, nach wie vielen Sekunden der jeweilige Bereichsausgang automatisch zurückgesetzt wird. Standard ist `0`, sodass ohne bewusste Konfiguration keine automatische Rücksetzung erfolgt; der Alarmausgang bleibt dann aktiv, bis er manuell zurückgesetzt oder der betreffende Bereich unscharf geschaltet wird. Beispielsweise entspricht `180` drei Minuten. Mehrere gleichzeitig ausgelöste Bereiche besitzen getrennte Ablaufzeitpunkte. Die zusammengefasste Instanzausgabe bleibt aktiv, solange mindestens ein Bereichsausgang aktiv ist. Die Rücksetzung beendet ausschließlich den jeweiligen Alarmausgang; Bereichszustand, Scharfmodus und Alarmgedächtnis bleiben gespeichert.

Für das Abschalten einer Sirene oder eines anderen dauerhaften Alarmgebers kann unter **Bei Rücksetzung des Alarmausgangs** eine eigene Symcon-Aktion hinterlegt werden. Sie wird erst ausgeführt, wenn auch der letzte aktive Bereichsausgang zurückgesetzt wurde. Eine automatische oder manuelle Rücksetzung wird zusätzlich mit `PartitionID` im Ereignisprotokoll als `alarm_output_reset` festgehalten.
...
Unter **Alarm-Eskalationsstufen** können zusätzliche Aktionen mit frei wählbaren Verzögerungen hinterlegt werden. Jede Stufe besitzt eine eigene Aktionsliste und kann damit mehrere Ausgänge, Leuchten oder weitere Ziele gleichzeitig ansteuern. Die Verzögerung jeder aktiven Stufe wird ab dem Beginn des gemeinsamen Alarmausgangs gemessen. Jede aktive Aktion einer fälligen Stufe wird in der konfigurierten Reihenfolge genau einmal ausgeführt; deaktivierte Einträge bleiben ohne Wirkung.
