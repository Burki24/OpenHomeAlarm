...
Die native Symcon-Kachel verwendet für die Kommunikation ausschließlich das HTML-SDK: Benutzeraktionen werden über `requestAction()` an `RequestAction()` des Moduls gesendet, während Statusänderungen über `UpdateVisualizationValue()` live an geöffnete Kacheln übertragen werden. Die statischen Dateien liegen unter `OpenHomeAlarm/visualization/` und werden über den zentralen `VisualizationAssetHelper` geladen; das gemeinsame Symcon-Design wird durch den vendorten `VisualizationThemeHelper` eingebettet. Die konfigurierbare IPSView-Farbpalette und ihre Kontrastkorrektur werden zentral durch den `IPSViewColorPaletteHelper` erzeugt.

Zusätzlich kann im Konfigurationsabschnitt **IPSView** eine eigenständige WebContent-Variable **IPSView-Alarmanlage** aktiviert werden. Sie verwendet dieselbe HTML-, CSS- und JavaScript-Oberfläche wie die native Kachel und ist vollständig bedienbar: Scharfmodi, Deaktivierung mit Code, Sensorüberbrückungen, Alarmgedächtnis und Rücksetzung des Alarmausgangs stehen auch in IPSView zur Verfügung. In IPSView muss die Variable in einer **HTML-Box** mit dem Renderer **Browser des Clients** dargestellt werden, da nur dieser Renderer JavaScript unterstützt. Transparenz, Farbschema und Schriftgröße können in der Modulkonfiguration angepasst werden.

Für das Farbschema **Benutzerdefinierte Farben** wird die komplette IPSView-Palette direkt in der Instanzkonfiguration festgelegt. Einstellbar sind Seitenhintergrund, normale und hervorgehobene Flächen, primäre und sekundäre Schriftfarbe, Akzentfarbe sowie die Statusfarben für Bereit/Aktiv, Warnung/Verzögerung und Alarm/Fehler. Die Farben werden mit `SelectColor` gespeichert und ohne IPSView-Farbvariablen an die WebContent-Seite übergeben. Weiche Hintergründe, Rahmen und Hover-Zustände werden daraus abgeleitet. Reicht der Kontrast für kleine Texte nicht aus, werden Flächen- beziehungsweise Schriftfarben nur so weit wie nötig in Richtung Schwarz oder Weiß korrigiert. Die Abschwächung nicht verfügbarer Bedienelemente und die bestehenden Statusverläufe bleiben erhalten. Die festen Schemata **Hell** und **Dunkel** bleiben weiterhin verfügbar.
