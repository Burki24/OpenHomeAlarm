...
Die native Symcon-Kachel verwendet für die Kommunikation ausschließlich das HTML-SDK: Benutzeraktionen werden über `requestAction()` an `RequestAction()` des Moduls gesendet, während Statusänderungen über `UpdateVisualizationValue()` live an geöffnete Kacheln übertragen werden. Die statischen Dateien liegen unter `OpenHomeAlarm/visualization/` und werden über den zentralen `VisualizationAssetHelper` geladen; das gemeinsame Symcon-Design wird durch den vendorten `VisualizationThemeHelper` eingebettet.

Zusätzlich kann im Konfigurationsabschnitt **IPSView** eine eigenständige WebContent-Variable **IPSView-Alarmanlage** aktiviert werden. Sie verwendet dieselbe HTML-, CSS- und JavaScript-Oberfläche wie die native Kachel und ist vollständig bedienbar: Scharfmodi, Deaktivierung mit Code, Sensorüberbrückungen, Alarmgedächtnis und Rücksetzung des Alarmausgangs stehen auch in IPSView zur Verfügung. In IPSView muss die Variable in einer **HTML-Box** mit dem Renderer **Browser des Clients** dargestellt werden, da nur dieser Renderer JavaScript unterstützt. Transparenz, Farbschema und Schriftgröße können in der Modulkonfiguration angepasst werden. Das Farbschema **An Umgebung anpassen** versucht die sichtbare Grund- und Akzentfarbe der umgebenden IPSView-Seite zu erkennen. Oberflächen werden daraus als halbtransparente, farblich getönte Flächen aufgebaut; kann der Client keine Hostfarbe bereitstellen, bleibt die Darstellung durch neutrale transparente Flächen mit dem Hintergrund verbunden. Die festen Schemata **Hell** und **Dunkel** stehen weiterhin zur Verfügung.

Für eine exakte Angleichung an eine bestehende IPSView können zusätzlich die dort bereits verwendeten String-Variablen für **Seiten-/Hintergrundfarbe**, **Element-/Flächenfarbe** und optional **Textfarbe** ausgewählt werden. IPSView erwartet für seine fernsteuerbaren Farben sechsstellige RGB-Hexadezimalwerte wie `4A1B07`; OpenHomeAlarm liest dieselben Werte und baut daraus kontrastgesicherte Flächen mit der gleichen Farbwelt. Änderungen an diesen Variablen werden automatisch übernommen. Die Verknüpfung ist nur im Farbschema **An Umgebung anpassen** aktiv. Wird keine Textfarbenvariable gewählt, verwendet die IPSView-Darstellung Weiß und dunkelt die Flächen bei Bedarf automatisch so weit ab, dass die Beschriftungen lesbar bleiben.
