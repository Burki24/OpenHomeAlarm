...
function jsonStyleCheck(string $dir, string $mode): array
{
...
function checkContentInFile(string $dir, string $mode): string|false
{
    $fileOriginal = file_get_contents($dir);
    if ($fileOriginal === false) {
        return $dir;
    }
    ...
    $fileCompare = json_encode(
        json_decode($fileOriginal, null, 512, JSON_THROW_ON_ERROR),
